 ...
 */ 
 ...
 [_QWERTY] = LAYOUT(
  ...
  KC_LCTL,  KC_Z,   KC_X,    KC_C,    KC_V,    KC_B, TD(TD_LLAVEI), TD(TD_LLAVEC),  KC_N,    KC_M,    TD(TD_MENOR), TD(TD_MAYOR),  TD(TD_SIGNO), TD(TD_PIPA),
                       KC_LALT, KC_LGUI, MO(_LOWER), KC_ENT,   KC_SPC, MO(_RAISE),KC_RALT, TD(TD_MAS)
),
/* LOWER
 * ,-----------------------------------------.                    ,-----------------------------------------.
 * | `/~  |      |      |      |      |      |                    |  F6  |  F7  |  F8  | F9   | F10  | F12  |
 ...
 * |      |      |      |      |      |      |                    |      |      |      |      |      | VOL+ |
 * |------+------+------+------+------+------|                    |------+------+------+------+------+------|
 * |LShift|      | CAPT |      | ctlrf|      |-------.    ,-------|      |      |  Up  |      |      | VOL- |
 * |------+------+------+------+------+------| supr  |    | PRINT |------+------+------+------+------+------|
 * |LCTRL |CTLZ  |      |CTL-C | CTL-V|      |-------|    |-------| CTLN | Left | Down |Rigth |      |  MUTE|
 * `-----------------------------------------/       /     \      \-----------------------------------------'
 *                   |  ALT | LGUI |LOWER | /Space  /       \Space \  |MAYUS |MENU  |MUTE  |
 *                   | LEFT |      |      |/       /         \      \ |      |      |      |
 *                   `----------------------------'           '------''--------------------'
 */
[_LOWER] = LAYOUT(
  ...
  KC_LSFT, _______, SGUI(KC_S),_______, RCTL(KC_F), _______,                   _______, _______,  KC_UP, _______, _______, KC_VOLD,
  KC_LCTL, LCTL(KC_Z), _______,  LCTL(KC_C), LCTL(KC_V),   _______, KC_DEL, KC_PSCR, LCTL(KC_N), KC_LEFT, KC_DOWN,KC_RGHT, _______, KC_MUTE,
                             KC_LALT, KC_TRNS, KC_SPC,  KC_SPC,  KC_SPC, KC_CAPS, KC_APP, _______
),
/* RAISE
 * ,-----------------------------------------.                    ,-----------------------------------------.
 * |  F11 |  F1  |  F2  |  F3  | alf4 | F5   |                    |      |      |      |      |      |Bri + |
 * |------+------+------+------+------+------|                    |------+------+------+------+------+------|
 * |      |      |      |      |      |      |                    |      |      |      |      |      |Bri - |
 * |------+------+------+------+------+------|                    |------+------+------+------+------+------|
 * |      |      |      |      |      |      |-------.    ,-------|      |      |      |win  L|      |RGB + |
...
[_RAISE] = LAYOUT(
  KC_F11,   KC_F1,    KC_F2,   KC_F3,  LALT(KC_F4),    KC_F5,                      _______, _______, _______, _______, _______, KC_BRIU,
...
switch (keycode) {
    case KC_CAPS:
        if (record->event.pressed) {
            ...
            // Enviar la pulsación de la tecla Caps Lock para que la máquina la reconozca
            tap_code(KC_CAPS);
            ...
            if (is_caps_lock_on) {
                ...
                rgblight_sethsv(85, 255, 255); // Cambia al color deseado
                rgblight_mode(RGBLIGHT_MODE_BREATHING + 3); // Ajusta el efecto según tu preferencia
            }

            // Asegúrate de salir de la capa LOWER, sin importar el estado de Caps Lock
            layer_off(_LOWER);
        }
        break;
}
